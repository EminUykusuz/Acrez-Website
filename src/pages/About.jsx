  ...
  return (
    <div className="bg-[#040f09] text-white min-h-screen pt-40 px-6 md:px-20 pb-20 relative overflow-hidden">
      ...
      <div className="mb-32 relative z-10">
        ...
        {/* Grid yapısı 3 karta göre güncellendi (lg:grid-cols-3) */}
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12 items-stretch">
          ...
          {/* Taha Onal (Yeni Eklenen Tasarımcı) */}
          <div className="bg-white/5 border border-white/10 p-10 rounded-[2rem] hover:border-acrez-accent/30 transition-all group flex flex-col justify-between">
            <div>
              <h3 className="text-3xl font-bold mb-2">{about.team?.designerName}</h3>
              <p className="text-acrez-accent text-sm tracking-widest uppercase font-bold mb-6">{about.team?.designerRole}</p>
              <p className="text-gray-400 text-lg leading-relaxed mb-6">
                {about.team?.designerDesc}
              </p>
            </div>
            <div className="flex gap-4 flex-wrap">
              {about.team?.designerSkills?.map((skill, index) => (
                <span key={index} className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-300">
                  {skill}
                </span>
              ))}
