...
export default function AdvancedPortfolio() {
  ...
  return (
    <section 
      ref={containerRef} 
      id="portfolio"
      // Mobilde adres çubuğu sorununu çözmek için h-[100dvh]
      className="relative w-full h-[100dvh] overflow-hidden bg-[#040f09]"
    >
      {projects.map((project, i) => {
        ...
        <div
          key={project.id}
          className="portfolio-panel absolute top-0 left-0 flex items-center justify-center w-full h-[100dvh] overflow-hidden"
          style={{ 
            backgroundColor: project.bg,
            zIndex: projects.length - i 
          }}
        >
          <div className="max-w-[90rem] w-full px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 items-center z-10">
            
            {/* METİN BÖLÜMÜ */}
            <div className={`content-section flex flex-col items-start text-left ${i % 2 !== 0 ? 'lg:order-2 lg:pl-12' : 'lg:order-1 lg:pr-12'}`}>
              <span className="inline-block px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-acrez-accent/10 border border-acrez-accent/20 text-acrez-accent text-xs md:text-sm tracking-[0.15em] uppercase font-bold mb-3 md:mb-6">
                {category}
              </span>
              
              <h2 className="text-4xl md:text-5xl lg:text-7xl font-black text-white uppercase tracking-tight mb-3 md:mb-6 leading-none">
                {title}
              </h2>

              <p className="text-gray-400 text-base md:text-lg lg:text-xl font-light leading-relaxed mb-6 md:mb-10 max-w-md">
                {desc}
              </p>
              ...
              {project.link && (
                <a 
                  ...
                  className="group inline-flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 rounded-full bg-white text-black text-sm md:text-base font-bold uppercase tracking-wide hover:bg-acrez-accent transition-colors duration-300"
                >
                  {viewBtnText}
                  <svg className="w-4 h-4 md:w-5 md:h-5 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* MOCKUP KOMPOZİSYONU */}
            <div className={`image-group relative w-full h-[35vh] sm:h-[45vh] lg:h-[60vh] flex items-center justify-center ${i % 2 !== 0 ? 'lg:order-1' : 'lg:order-2'}`}>
              ...
              <div className="relative w-[90%] md:w-[85%] aspect-[16/10] bg-[#0a0a0a] rounded-xl overflow-hidden border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col z-10">
                <div className="h-6 md:h-10 w-full bg-[#1c1c1e] flex items-center px-2 md:px-4 gap-1.5 md:gap-2 shrink-0 z-20">
                ...
                <div className="relative w-full h-full bg-[#0a0a0a]">
                  <img src={project.image} alt={`${project.title} Desktop`} className="absolute inset-0 w-full h-full object-cover object-top" />
                </div>
              </div>

              {/* MOBİL MOCKUP */}
              <div className={`absolute -bottom-2 md:-bottom-12 ${i % 2 === 0 ? '-right-1 md:-right-12' : '-left-1 md:-left-12'} 
                            w-[32%] md:w-[26%] aspect-[9/19] bg-[#0a0a0a] rounded-xl md:rounded-3xl overflow-hidden 
                            border-[4px] md:border-[8px] border-[#161616] z-20`}
              >
                <div className="absolute top-1 md:top-3 left-1/2 -translate-x-1/2 h-2 md:h-4 w-[30%] bg-[#050505] rounded-full z-30 border border-white/5"></div>
                <div className="relative w-full h-full">
                  <img src={project.mobileImage} alt={`${project.title} Mobile`} className="absolute inset-0 w-full h-full object-cover object-top" />
                </div>
              </div>
